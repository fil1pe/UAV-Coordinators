...
namespace Coordinator
{
    public partial class CommunicationLinks : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            row.Add(cbxType.Text);
            row.Add(txtIP.Text);
            row.Add(txtPort.Text);
            database.AddRow(row);

            DT.Rows.Add(txtName.Text, cbxType.Text, txtIP.Text, txtPort.Text);
            ...
            while (!proc.HasExited)
            {
                if (!sr.EndOfStream)
                {
                    ...
                    try
                    {
                        ...
                        map.MapControl.GetUavById(indexx).CurrentPosition = new PointLatLng(lat, lng);
